...
def translasteLongReason(t):
    switch = {
        ...
        '65': '''The Network Access Permission setting in the dial-in properties of the user account is set to Deny access to the user.
        ...
        '72': 'The user password has expired or is about to expire and the user must change their password, however Authentication Methods in network policy constraints are not configured to allow the user to change their password. To allow the user to change their password, open the properties of the matching network policy, click the Constraints tab, click Authentication Methods, and then in the details pane select the appropriate authentication method and User can change password after it has expired check box.',
...
def translateReason(t):
    ...
    if int(t) in switch:
        return switch[int(t)]
    return translasteLongReason(t)


def translatePackageType(t):
    switch = {
        '1': 'Request',
        '2': 'Accepted ',
        '3': 'Rejected',
